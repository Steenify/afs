...
class ListContact extends Component {
  ...
  render() {
    ...
    const { onSave, uid, placeholder } = this.props;
    return (
        ...
        <div className='order__artist'>
          ...
          <div className='list mb-3'>
            {isLoading ? (
              ...
              <div>
                <button onClick={() => onSave({ exId: '' })} key={`list__artist__login`} className={`artist__select `}>
                  <strong className='name'>____Select____</strong>
                  <div className='status'></div>
                </button>
                {contacts.map((cont) => {
                  return (
                    <button onClick={() => onSave(cont)} key={`list__artist__${cont.id}`} className={`artist__select ${cont.exId === uid ? 'active' : ''}`}>
                      <div className='avt'>
                        <img className='w-100 h-100' style={{ objectFit: 'cover' }} src={cont.avatarUrl} alt='contact__author' />
                      </div>

                      <div className='info'>
                        <strong className='name'>{cont?.fullName || `${cont?.firstName} ${cont?.lastName}`}</strong>
