...
import { updateOrderTableBudgetAction } from 'components/tables/orders/actions';
...
class OrderBudget extends Component {
  ...
  onSave = (e) => {
    ...
    const { order, updateOrderTableBudgetAction } = this.props;
    ...
    this.setState(
      ...
      () => {
        ...
        updateOrderTableBudgetAction({
          payload,
          id: order.id,
          onSuccess: () => toast.dark(`Order [#${order?.number}]'s budget is changed to ${formatMoney(number || 0)}$`),
        });
      },
    );
  };
  ...
  render() {
    ...
    return (
      <Popover
            ...
            <form action='' onSubmit={this.onSave}>
              <div className='order__budget'>
                <div className='data'>
                  ...
                  <NumberFormat prefix={'$  '} thousandSeparator={true} className='form-control bugdet__input' value={value} onValueChange={this.onChange} />
                </div>
                <div className='ctas'>
                  <Button onClick={this.toggle} className='bugdet__cancel cta pl-0' type='button' color='link'>
                    Cancel
                  </Button>
                  <Button onClick={this.onSave} className='bugdet__save cta pr-0' type='button' color='link'>
                    Save
                  </Button>
                </div>
              </div>
        ...
        <button onClick={this.toggle} className='order__toggle order__budget budget p-0'>
