...
import { getListImageUrl, dateTimeStringFromDate, getOrderItem, getOrderOption } from 'utils';

const OrderItemBox = ({ item, order, hasFaster }) => {
  return (
    <div className='order_detail__original order_detail__box box'>
      <div className='box__header'>
        <div className='box__title'>Order Original {hasFaster && '(Faster Processing)'}</div>
      ...
      <div className='box__body'>
        ...
        {getOrderOption(item.name) && (
          <div className='options'>
            {getOrderOption(item.name)} ({item?.quantity})
          </div>
        )}
