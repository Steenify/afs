...
const ArtistWorkingAttitudeCell = ({ workingAttitudeScore, id, login, updateArtistItems, updateArtistDetailApi }) => {
  const [value, setValue] = useState(workingAttitudeScore || '');

  const [isPopoverOpen, setIsPopoverOpen] = useState(false);
  const toggle = () => setIsPopoverOpen(!isPopoverOpen);

  useEffect(() => {
    setValue(workingAttitudeScore || '');
            ...
            <div className='order__budget'>
              <div className='data'>
                ...
                <select className='form-control' value={value} onChange={onChange}>
                  ...
                  {scores.map((sco) => {
                    return (
                      <option key={`artist__${id}__select__score_attitute__${sco}`} value={sco + 1}>
                        {sco + 1}
                      </option>
                    );
                  })}
...
const mapStateToProps = ({ artists, auth }, ownProps) => {
  ...
  return {
    workingAttitudeScore: item?.workingAttitudeScore || '',
    id: item?.id,
    login: item?.login || 0,
...
export default connect(mapStateToProps, mapDispatchToProps)(ArtistWorkingAttitudeCell);
